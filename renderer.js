...
if (campoURL) {
  ...
  document.getElementById('maximizar').onclick = () => {
    window.electronAPI.maximizar();
  };

  document.getElementById('cerrar').onclick = () => {
    window.electronAPI.cerrar();
  };

  window.addEventListener('DOMContentLoaded', () => {
    const barra = document.querySelector('.browser-controls');
    if (barra) {
      const altura = barra.offsetHeight + 40; // 40px de margen extra
      window.electronAPI.ajustarAltura(altura);
    }
  });
}
